...
public static class Extensions
{
    ...
    public static FurniData FurniData => _furniData ?? throw new InvalidOperationException($"{typeof(Extensions).FullName} has not been initialized.");
    public static ExternalTexts Texts => _texts ?? throw new InvalidOperationException($"{typeof(Extensions).FullName} has not been initialized.");
    ...
    /// </summary>
    public static void Initialize(FurniData? furniData)
    {
        _furniData = furniData;
        IsInitialized = furniData is not null;
    }
    ...
    /// </summary>
    public static string GetNameOrDefault(this IItem item)
    {
        ...
        return item.Type switch
        {
            ...
            _ => string.IsNullOrWhiteSpace(variant) ? $"{item.Type}:{item.TypeID}" : $"{item.Type}:{item.Id}:{variant}"
        };
    }

    public static FurniInfo GetByType(ItemType type, long TypeID)
    {
        if (type == ItemType.Floor)
        {
            foreach (var item in FurniData.FloorItems)
            {
        ...
        {
            foreach (var item in FurniData.WallItems)
            {
                if (item.TypeID == TypeID)
                {
                    return item;
    ...
    public static FurniInfo GetByType(this IItem item)
    {
        return GetByType(item.Type, item.TypeID);
    }
    ...
    public static FurniInfo GetInfoFromType(this IItem item) => FurniData.GetInfoFromType(item);
    ...
    public static ItemDescriptor GetDescriptor(this IItem item)
    {
        ...
        return new ItemDescriptor(item.Type, item.TypeID, item.Identifier, variant);
    }
    ...
    public static IEnumerable<T> OfKind<T>(this IEnumerable<T> items, FurniInfo furniInfo)
        where T : IItem => OfKind(items, new ItemDescriptor(furniInfo.Type, furniInfo.TypeID, null, null));
    ...
    {
        var set = new HashSet<(ItemType, long)>(furniInfo.Select(info => (info.Type, info.TypeID)));
        return items.Where(item => set.Contains((item.Type, item.TypeID)));
    }
        ...
        where T : IItem
    {
        return items.Where(item =>
            FurniData.TryGetInfo(item, out FurniInfo? info) &&
            info.ClassName.Equals(identifier, StringComparison.OrdinalIgnoreCase)
        );
    }
    ...
    /// <summary>
    /// Selects items beinting to the specified category.
    /// </summary>
    ...
    /// <summary>
    /// Selects items beinting to any of the specified categories.
    /// </summary>
    ...
    /// <summary>
    /// Selects items not beinting to the specified category.
    /// </summary>
    ...
    /// <summary>
    /// Selects items not beinting to any of the specified categories.
    /// </summary>
    ...
    /// <summary>
    /// Selects items beinting to the specified furni line.
    /// </summary>
    ...
    /// <summary>
    /// Selects items beinting to any of the specified furni lines.
    /// </summary>
    ...
    /// <summary>
    /// Selects items not beinting to the specified furni line.
    /// </summary>
    ...
    /// <summary>
    /// Selects items not beinting to any of the specified furni lines.
    /// </summary>
    ...
    /// </summary>
    public static IEnumerable<T> OfKind<T>(this IEnumerable<T> items, long TypeID)
        where T : IFloorItem, IWallItem => items.Where(item => item.TypeID == TypeID);

    /// <summary>
    /// Gets items of the specified kinds.
    /// </summary>
    public static IEnumerable<T> OfKinds<T>(this IEnumerable<T> items, IEnumerable<long> TypeIds)
        where T : IFloorItem, IWallItem
    {
        HashSet<long> set = new(TypeIds);
        return items.Where(item => set.Contains(item.TypeID));
    }

    /// <summary>
    /// Gets items of the specified kinds.
    /// </summary>
    public static IEnumerable<T> OfKinds<T>(this IEnumerable<T> items, params long[] TypeIDs)
        where T : IFloorItem, IWallItem => OfKinds<T>(items, TypeIDs);

    /// <summary>
    /// Gets items owned by the specified user ID.
    /// </summary>
    public static IEnumerable<T> OwnedBy<T>(this IEnumerable<T> items, int ownerId)
    ...
    public static IEnumerable<IGrouping<int, IInventoryItem>> Group(this IEnumerable<IInventoryItem> items, int maxSlots = 9, int maxItems = 1500)
    {
        ...
        var lastKind = ((ItemType)(-1), unchecked((long)-1));
        ...
        return items
            ...
            .GroupBy(item =>
            {
                var TypeID = (item.Type, item.TypeID);
                if (!item.IsGroupable || lastKind != TypeID)
                    currentSlots++;
                currentCount++;
                lastKind = TypeID;
        ...
        where TFloorAvatar : IFloorEntity
    {
        ...
        {
            Area a = x.Area.GetValueOrDefault();
    ...
    public static IEnumerable<TFloorAvatar> Outside<TFloorAvatar>(this IEnumerable<TFloorAvatar> avatars, AreaSet areas)
        where TFloorAvatar : IFloorEntity
    {
        return avatars.Where(x => !areas.Contains(x.Area.GetValueOrDefault()));
    }
    #endregion
}
