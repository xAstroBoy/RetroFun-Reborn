...
public sealed record SearchGroupMembersMsg(
    int Id, int Page = 0, string Filter = "", GroupMemberSearchType SearchType = GroupMemberSearchType.Members
)
    : IRequestMessage<SearchGroupMembersMsg, GroupMembersMsg, GroupMembers>
{
    static ClientType IMessage<SearchGroupMembersMsg>.SupportedClients => ClientType.Nitro;
    static Identifier IMessage<SearchGroupMembersMsg>.Identifier => Out.Group_Members;
