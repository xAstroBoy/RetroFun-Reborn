    ...
    : GameStateManager(extension)
{
    ...
    private bool TryStartTradeModern(int traderId, int tradeeId)
    {
    ...
    private bool TryStartTrade(IUser trader, IUser tradee)
    {
        ...
        if (_profileManager.UserData is not { Id: int selfId, Name: string selfName })
        {
    ...
    private void UserAccepted(int? userId, string? userName, bool accepted)
    {
    ...
    private void CloseTrade(int? userId, int? reason)
    {
        if (!IsTrading) return;
