...
public sealed partial class FriendManager(IInterceptor interceptor, ILoggerFactory? loggerFactory = null) : GameStateManager(interceptor)
{
    ...
    public bool IsFriend(int userId) => _friends.ContainsKey(userId);
    ...
    public IFriend? GetFriend(int userId) => _friends.TryGetValue(userId, out Friend? friend) ? friend : null;
    ...
    /// </summary>
    public void SendMessage(int userId, string message) => Interceptor.Send(new SendConsoleMessageMsg { Recipients = [userId], Message = message });
    ...
    protected override void OnInitialize(bool initializingOnConnect)
    {
        if (!_receivedMessengerInit)
        {
            ...
            Interceptor.Send(Out.Messenger_Init);
        }
    }
    ...
    private void ReceiveFriendRequest(int id, string name, string? figureString)
    {
