...
internal class Room : IRoom, INotifyPropertyChanged
{
    ...
    internal ConcurrentDictionary<int, FloorItem> FloorItems { get; } = new();
    internal ConcurrentDictionary<int, WallItem> WallItems { get; } = new();

    internal ConcurrentDictionary<int, Avatar> Avatars { get; } = new();

    public Room(int id, RoomData? roomData)
    {
        Id = id;
        _roomData = roomData;
    }
    ...
    /// </summary>
    public bool HasFloorItem(int itemId) => FloorItems.ContainsKey(itemId);

    /// <summary>
    /// Gets whether the wall item with the specified ID exists in the room or not.
    /// </summary>
    public bool HasWallItem(int itemId) => WallItems.ContainsKey(itemId);

    /// <summary>
    /// Attempts to get the furni of the specified type with the specified ID and returns <c>true</c> if successful.
    /// </summary>
    public bool TryGetFurni(ItemType type, int itemId, [NotNullWhen(true)] out IFurni? furni)
    ...
    /// </summary>
    public bool TryGetFloorItem(int itemId, [NotNullWhen(true)] out IFloorItem? item)
    ...
    /// </summary>
    public bool TryGetWallItem(int itemId, [NotNullWhen(true)] out IWallItem? item)
    ...
    /// </summary>
    public IFurni? GetFurni(ItemType type, int FurniID)
    {
        return type switch
        {
            ItemType.Floor => GetFloorItem(FurniID),
            ItemType.Wall => GetWallItem(FurniID),
    ...
    public TAvatar? GetAvatarById<TAvatar>(int id) where TAvatar : IAvatar
    {
        return Avatars
            .Select(x => x.Value)
            .OfType<TAvatar>()
    ...
    public IUser? GetUserById(int id) => Avatars.Values.OfType<IUser>().FirstOrDefault(x => x.Id == id);

    public bool TryGetAvatarByIndex(int index, [NotNullWhen(true)] out IAvatar? avatar) => (avatar = GetAvatar(index)) is not null;

    public bool TryGetAvatarById<TAvatar>(int id, [NotNullWhen(true)] out TAvatar? avatar) where TAvatar : IAvatar
    {
