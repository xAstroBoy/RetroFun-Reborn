...
public class Inventory : IInventory, IEnumerable<InventoryItem>
{
    ...
    public InventoryItem? GetItem(int id) => TryGetItem(id, out InventoryItem? item) ? item : null;
    IInventoryItem? IInventory.GetItem(int id) => GetItem(id);
    public bool TryGetItem(int itemId, [NotNullWhen(true)] out InventoryItem? item) => _items.TryGetValue(itemId, out item);
    bool IInventory.TryGetItem(int id, [NotNullWhen(true)] out IInventoryItem? item) => (item = GetItem(id)) is not null;

    public bool TryRemove(int itemId, [NotNullWhen(true)] out InventoryItem? item) => _items.TryRemove(itemId, out item);
