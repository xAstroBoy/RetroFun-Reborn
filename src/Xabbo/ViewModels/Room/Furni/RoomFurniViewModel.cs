...
namespace Xabbo.ViewModels
{
    public partial class RoomFurniViewModel : ViewModelBase
    {
        ...
        // **Added Properties for Search Field Selection**
        public ReadOnlyObservableCollection<string> SearchFields { get; }
        [Reactive] public string SelectedSearchField { get; set; } = "Name";

        // **Added Description Property**
        public string Description => ContextSelection != null && ContextSelection.Any()
            ? string.Join(Environment.NewLine + Environment.NewLine, ContextSelection.Select(f => f.Description))
            ...
            RoomGiftsViewModel gifts)
        {
            ...
            var searchFieldsList = new List<string> { "Name", "Owner", "ID", "Description" };
            var searchFieldsObservable = new ReadOnlyObservableCollection<string>(new ObservableCollection<string>(searchFieldsList));
            SearchFields = searchFieldsObservable;

            var nameAndQueryFilter = this.WhenAnyValue(
                x => x.FilterText,
                x => x.FilterArea,
                x => x.SelectedSearchField,
                (filterText, filterArea, selectedSearchField) => {
                    ...
                    return (nameFilterText, queryFilterText, filterArea, selectedSearchField);
                }
            );

            _nameFilter = nameAndQueryFilter
                .Select(x => CreateNameFilter(x.nameFilterText, x.selectedSearchField))
                .ToProperty(this, x => x.NameFilter);

            var queryFilterAndError = nameAndQueryFilter
                .Select(x => CreateQueryFilter(x.queryFilterText, x.filterArea, x.selectedSearchField));
            ...
            _isDynamicFilterEnabled =
                this.WhenAnyValue(
                    x => x.QueryFilter,
                    x => x.FilterArea,
                    (queryFilter, filterArea) =>
                        queryFilter is not null || filterArea.HasValue
                )
        ...
        // **Updated CreateNameFilter to include selected search field**
        static Func<FurniViewModel, bool> CreateNameFilter(string? nameFilter, string selectedSearchField)
        {
            ...
            switch (selectedSearchField)
            {
                ...
                case "Owner":
                    return (vm) =>
                        string.IsNullOrWhiteSpace(nameFilter) ||
                        vm.Owner.Contains(nameFilter, StringComparison.OrdinalIgnoreCase);
                case "ID":
                    if (long.TryParse(nameFilter, out long id))
                    {
                        return (vm) => vm.Id == id;
                    }
                    else
                ...
                case "Description":
                    return (vm) =>
                        string.IsNullOrWhiteSpace(nameFilter) ||
                        vm.Description.Contains(nameFilter, StringComparison.OrdinalIgnoreCase);
                default:
                    return static (vm) => true;
            }
        }

        // **Updated CreateQueryFilter to include selected search field**
        static (Func<FurniViewModel, bool>? Filter, string? Error) CreateQueryFilter(string? expression, Area? filterArea, string selectedSearchField)
        {
        ...
        {
            // **Keeping existing syntax as per user request**
